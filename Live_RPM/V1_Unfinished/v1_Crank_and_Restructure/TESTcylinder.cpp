...
// Cylinder elements
typedef enum { INTAKE_VALVE = 0, EXHAUST_VALVE = 1, FUEL_INJECTOR = 2, SPARK_PLUG = 3, PISTON = 4} Elements;
// Array sizes
...
void updateCylinderElements(unsigned int currentState) {
	
	for (int i = 0; i < CYLINDER_ELEMENTS; ++i) {
		cylinderElementStates[i] = cylinderStatesMatrix[i][currentState]; 
	}
}
